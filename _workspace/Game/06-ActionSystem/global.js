...
Action.globalFilter = function (id) {
	...
	const noTargetType = ["General", "Magic", "Item", "Onani", "Other", "System"];

	//总之先按照分类过滤
	if (
		!groupmatch(data.type, "Menu", "General", "System") &&
	...
	if (pc == "m0" && !groupmatch(data.type, "Tentacles", "System", "Menu", "Other")) return 0;
...
Action.globalOrder = function (id) {
	const { type, setting = "" } = Action.data[id];
	let order = 0;
